...
class TipSplitterApp(ctk.CTk):
    def __init__(self):
        ...
        self.geometry("1235x445")
        self.iconbitmap("./imgs/wizard.ico")
        ...
        self.splitting_tips_label.grid(row=1, column=0, padx=20, pady=10)
        self.splitting_tips_entry = ctk.CTkEntry(self.sidebar_frame)
        self.splitting_tips_entry.grid(row=2, column=0, padx=20, pady=10)

        self.total_tips_label = ctk.CTkLabel(self.sidebar_frame, text="Total Tips:")
        self.total_tips_label.grid(row=3, column=0, padx=20, pady=10)
        self.total_tips_entry = ctk.CTkEntry(self.sidebar_frame)
        self.total_tips_entry.grid(row=4, column=0, padx=20, pady=10)
        ...
        self.export_button = ctk.CTkButton(self.sidebar_frame, text="Export", fg_color="transparent", border_width=2, command=self.export)
    ...
    def add_worker(self):
        ...
        position_label = ctk.CTkLabel(worker_frame, text="Position:")
        position_label.grid(row=0, column=2, padx=5, pady=3)
        position_optionmenu = ctk.CTkOptionMenu(worker_frame, values=["Bartender", "Bar Back", "Other"])
        position_optionmenu.grid(row=0, column=3, padx=5, pady=3)

        point_label = ctk.CTkLabel(worker_frame, text="Points:")
        point_label.grid(row=0, column=4, padx=5, pady=3)
        point_optionmenu = ctk.CTkOptionMenu(worker_frame, values=["1", ".75", ".5", ".25"])
        point_optionmenu.grid(row=0, column=5, padx=5, pady=3)
        ...
        self.worker_entries.append((worker_frame, name_entry, position_optionmenu, point_optionmenu, notes_entry, tipout_entry))
    ...
    def calculate_tipout(self):
        total_tips = float(self.total_tips_entry.get())

        # Calculate the total point value of all workers
        total_points = 0.0
        for _, _, _, point_optionmenu, _, _ in self.worker_entries:
            point = float(point_optionmenu.get())
            total_points += point

        # Calculate the tipout for each worker
        for _, _, _, point_optionmenu, _, tipout_entry in self.worker_entries:
            point = float(point_optionmenu.get())
            tipout = total_tips * (point / total_points)
    ...
    def export(self):
        ...
        total_tips = self.total_tips_entry.get()
        ...
        workers = []
        for _, name_entry, position_optionmenu, point_optionmenu, notes_entry, tipout_entry in self.worker_entries:
            name = name_entry.get()
            position = position_optionmenu.get()
            point = point_optionmenu.get()
            tipout = tipout_entry.get()
            notes = notes_entry.get()

            if not name or not position or not point:
                CTkMessagebox(message ="Please fill in all fields for all workers.", title="Error", icon='cancel')
                return
        ...
        # You can replace the print statements with the desired export functionality
